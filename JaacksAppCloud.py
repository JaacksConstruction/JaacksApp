...
# Helper: full contractor name consistently
def _full_name(df_row):
    fn = str(df_row.get('FirstName', '')).strip()
    sn = str(df_row.get('Surname', '')).strip()
    return (fn + ' ' + sn).strip()
...
if section == 'User Management':
    if current_user_role_val == 'Admin':
        ...
        with st.form("new_user_form_um", clear_on_submit=True):
            ...
            if new_role_select_um == "Client Viewer":
                clients_list_create_um = ["Select Client"] + sorted(
                    list(jobs_df['Client'].astype(str).str.strip().replace('', np.nan).dropna().unique())
                )
            ...
            if st.form_submit_button("Create User"):
                if not all([new_fname_um, new_sname_um, new_pass_create_um, confirm_pass_create_um]) or \
                   (new_role_select_um == "Client Viewer" and assoc_client_input_create_um == "Select Client"):
                ...
                elif new_pass_create_um != confirm_pass_create_um:
                    ...
                    new_user_data_um = {
                        'Username': uname_final_um,
                        'PasswordHash': phash_create_um,
                        'Salt': salt_create_um,
                        'Role': new_role_select_um,
                        'FirstName': new_fname_um.strip().title(),
                        'Surname': new_sname_um.strip().title(),
        ...
        display_paginated_dataframe(
            users_df_manage_section[['Username', 'FirstName', 'Surname', 'Role', 'AssociatedClientName']],
            "users_page_display_um", 10
        )

        # --- Edit User Details ---
        st.markdown("---"); st.subheader("Edit User Details")
        edit_user_options_list_um = ["Select User to Edit"] + sorted(
            users_df_manage_section.apply(lambda r: f"{_full_name(r)} ({r['Username']})", axis=1).tolist()
        )
        ...
        if selected_user_to_edit_display_um != "Select User to Edit":
            ...
            if not user_to_edit_data_series_um.empty:
                ...
                with st.form(f"edit_user_form_{user_uid_for_edit_um}", clear_on_submit=False):
                    st.write(f"Editing User: {_full_name(user_data_for_edit_um)} ({user_data_for_edit_um['Username']})")
                    ...
                    if edit_role_um_val == "Client Viewer":
                        clients_list_form_um = ["Select Client"] + sorted(
                            list(jobs_df['Client'].astype(str).str.strip().replace('', np.nan).dropna().unique())
                        )
                        current_assoc_idx_um = clients_list_form_um.index(assoc_client_current_um) if assoc_client_current_um in clients_list_form_um else 0
                        ...
                        if not all([edit_fname_um_val, edit_sname_um_val]) or \
                           (edit_role_um_val == "Client Viewer" and final_assoc_client_edit_um == "Select Client"):
                            ...
                            if not idx_update_um.empty:
                                ...
                                st.error("Failed to find user to update. Please refresh.")
            else:
                st.error("Selected user details could not be retrieved. Please refresh.")

        # --- Delete User ---
        st.markdown("---"); st.subheader("Delete User")
        delete_options_um = ["Select User to Delete"] + sorted([
            f"{_full_name(r)} ({r['Username']})"
        ...
        if selected_user_to_delete_disp_um != "Select User to Delete":
            ...
            if not user_to_delete_series_um.empty:
                ...
                if st.button(f"Request Delete User: {_full_name(user_data_delete_um)}", key=f"req_del_user_btn_{user_uid_delete_um}"):
                ...
                if st.session_state.get(f"confirm_del_user_flag_{user_uid_delete_um}", False):
                    st.warning(f"Delete user: **{_full_name(user_data_delete_um)} ({user_data_delete_um['Username']})**? This is irreversible.")
        ...
        if not users_df_manage_section.empty:
            user_names_reset_um = ["Select User to Reset Password"] + sorted(
                users_df_manage_section.apply(lambda r: f"{_full_name(r)} ({r['Username']})", axis=1).tolist()
            )
            ...
            if selected_user_disp_reset_um != "Select User to Reset Password":
                ...
                if not user_record_reset_um_series.empty:
                    ...
                    with st.form(f"pwd_reset_form_{user_data_reset_um['UserUniqueID']}"):
                        st.write(f"Resetting password for: **{_full_name(user_data_reset_um)}**")
                        ...
                        if st.form_submit_button("Reset Password"):
                            if not new_pwd_reset_um or not confirm_pwd_reset_um:
                                st.error("Enter and confirm the new password.")
                            elif new_pwd_reset_um != confirm_pwd_reset_um:
                                st.error("Passwords do not match.")
                            else:
                                ...
                                if not idx_update_pwd_um.empty:
                                    ...
                                    st.error("User not found for password reset during save. Refresh.")
                else:
                    st.error("User details for password reset not found. Refresh.")
        else:
            st.info("No users available to reset passwords.")
    else:
    ...
    # Determine dashboard view based on role and filters
    ...
    if current_user_role_val == 'Client Viewer' and associated_client_name_val:
        dashboard_jobs_view_df = dashboard_jobs_view_df[
            dashboard_jobs_view_df['Client'].astype(str).str.strip() == associated_client_name_val.strip()
        ]
        ...
        job_names_client_viewer = ["All My Jobs"] + sorted(
            list(dashboard_jobs_view_df['Job Name'].astype(str).str.strip().replace('', np.nan).dropna().unique())
        )
        ...
        if job_filter_for_dashboard == "All My Jobs":
            job_filter_for_dashboard = "All Jobs"

    else:  # Admin, Manager, Contractor
        all_clients_list_dash = ["All Clients"] + (sorted(
            list(jobs_df['Client'].astype(str).str.strip().replace('', np.nan).dropna().unique())
        ...
        if client_filter_for_dashboard != "All Clients":
            jobs_for_client_filter_dash = jobs_for_client_filter_dash[
                jobs_for_client_filter_dash['Client'].astype(str).str.strip() == client_filter_for_dashboard.strip()
            ]

        all_jobs_list_dash = ["All Jobs"] + (sorted(
            list(jobs_for_client_filter_dash['Job Name'].astype(str).str.strip().replace('', np.nan).dropna().unique())
        ) if not jobs_for_client_filter_dash.empty else [])
        ...
        job_filter_for_dashboard = st.selectbox(
            f"Filter by Job ({client_filter_for_dashboard if client_filter_for_dashboard != 'All Clients' else 'any client'}):",
    ...
    if search_query:
        ...
        jobs_display_jd = jobs_df[
            jobs_df['Job Name'].astype(str).str.lower().str.contains(search_query_lower) |
            jobs_df['Client'].astype(str).str.lower().str.contains(search_query_lower)
        ]
    ...
    display_paginated_dataframe(
        jobs_display_jd.sort_values(by="Start Date", ascending=False),
        "jd_page_display",
        styler_fn=highlight_job_deadlines,
        col_config={
            "UniqueID": None,
    ...
    if current_user_role_val == 'Admin':
        ...
        job_to_edit_options = ["Select..."] + sorted(list(jobs_display_jd['Job Name'].astype(str).unique()))
        ...
        if job_to_edit_select_admin_jd != "Select...":
            ...
            if not job_data_series_admin_jd.empty:
                ...
                with st.form(f"edit_job_form_{job_uid_edit_admin_jd}"):
                    ...
                    edit_name_val = st.text_input("Job Name", value=str(job_data_edit_admin_jd['Job Name']), key=f"ej_name_{job_uid_edit_admin_jd}")
                    edit_client_val = st.text_input("Client", value=str(job_data_edit_admin_jd['Client']), key=f"ej_client_{job_uid_edit_admin_jd}")

                    st.write("Edit Client Address")
                    c1_edit, c2_edit = st.columns(2)
                    address_jd_edit = c1_edit.text_input("Street Address", value=str(job_data_edit_admin_jd.get('ClientAddress', '')), key=f"ej_addr_{job_uid_edit_admin_jd}")
                    city_jd_edit = c2_edit.text_input("City", value=str(job_data_edit_admin_jd.get('ClientCity', '')), key=f"ej_city_{job_uid_edit_admin_jd}")
                    s1_edit, s2_edit = st.columns(2)
                    state_jd_edit = s1_edit.text_input("State", value=str(job_data_edit_admin_jd.get('ClientState', '')), key=f"ej_state_{job_uid_edit_admin_jd}")
                    zip_jd_edit = s2_edit.text_input("Zip Code", value=str(job_data_edit_admin_jd.get('ClientZip', '')), key=f"ej_zip_{job_uid_edit_admin_jd}")

                    status_opts_ej = ["Planning", "In Progress", "On Hold", "Completed", "Cancelled"]
                    current_status = str(job_data_edit_admin_jd['Status'])
                    status_idx_ej = status_opts_ej.index(current_status) if current_status in status_opts_ej else 0
                    ...
                    edit_desc_val = st.text_area("Description", value=str(job_data_edit_admin_jd.get('Description', '')), key=f"ej_desc_{job_uid_edit_admin_jd}")
                    edit_eh_val = st.number_input("Est. Hours", value=float(job_data_edit_admin_jd.get('Estimated Hours', 0.0)), format="%.1f", key=f"ej_eh_{job_uid_edit_admin_jd}")
                    edit_emc_val = st.number_input("Est. Mat. Cost ($)", value=float(job_data_edit_admin_jd.get('Estimated Materials Cost', 0.0)), format="%.2f", key=f"ej_emc_{job_uid_edit_admin_jd}")

                    save_col_ej_btn, del_col_ej_btn = st.columns(2)
                    if save_col_ej_btn.form_submit_button("Save Changes"):
                        if not edit_name_val or not edit_client_val:
                            st.error("Job Name and Client are required.")
                        else:
                            ...
                            if not idx_update_ej_q.empty:
                                ...
                                st.error("Job not found for update. Refresh.")
    ...
    # Consistent contractor list: full names
    if not users_df.empty:
        users_with_full = users_df.copy()
        users_with_full['FullName'] = users_with_full.apply(_full_name, axis=1)
        contractor_names_for_filter_jtt = users_with_full[users_with_full['Role'].isin(['Contractor', 'Admin', 'Manager'])]['FullName'].astype(str).str.strip().unique()
    else:
        contractor_names_for_filter_jtt = []
    ...
    if current_user_role_val == 'Contractor':
        time_df_display_jtt = time_df_display_jtt[time_df_display_jtt['Contractor'].astype(str).str.strip() == current_user_fullname_val]
        ...
        if selected_contractor_jtt_disp_filter != "All Contractors":
            time_df_display_jtt = time_df_display_jtt[time_df_display_jtt['Contractor'].astype(str).str.strip() == selected_contractor_jtt_disp_filter]
    else:  # Admin, Manager
        selected_contractor_jtt_disp_filter = st.selectbox("Filter by Contractor:", options=contractor_choices_jtt_filter,
    ...
    if selected_job_jtt_disp_filter != "All Jobs":
        time_df_display_jtt = time_df_display_jtt[time_df_display_jtt['Job'].astype(str).str.strip() == selected_job_jtt_disp_filter]
    ...
    if current_user_role_val in ['Contractor', 'Admin', 'Manager']:
        ...
        with st.form("new_time_entry_form_jtt", clear_on_submit=True):
            ...
            if current_user_role_val == 'Contractor':
                ...
                if not assignable_contractors_jtt:
                    st.warning("No contractors available to assign time entry.")
                else:
                    contractor_for_new_entry_jtt = st.selectbox("Contractor*", options=assignable_contractors_jtt, key="jtt_new_time_contractor_select")
            ...
            if selected_job_new_entry_jtt and selected_job_new_entry_jtt != "Select Job":
                ...
                st.text_input("Client (Auto-filled)", value=str(client_for_new_entry_jtt), disabled=True, key="jtt_new_time_client_auto")
            else:
            ...
            if st.form_submit_button("Add Time Entry"):
                if not (contractor_for_new_entry_jtt and selected_job_new_entry_jtt != "Select Job" and
                    ...
                    duration_jtt = (datetime.datetime.combine(date_new_entry_jtt, end_time_new_entry_jtt) -
                                    datetime.datetime.combine(date_new_entry_jtt, start_time_new_entry_jtt)).total_seconds() / 3600.0

                    new_entry_record_jtt = {
                        'Contractor': str(contractor_for_new_entry_jtt).strip(),
                        'Client': str(client_for_new_entry_jtt).strip(),
                        'Job': str(selected_job_new_entry_jtt).strip(),
                        'Date': date_new_entry_jtt,
                        'Start Time': start_time_new_entry_jtt.strftime('%H:%M'),
                        'End Time': end_time_new_entry_jtt.strftime('%H:%M'),
                        'Time Duration (Hours)': float(duration_jtt),
                        'UniqueID': uuid.uuid4().hex,
    ...
    display_paginated_dataframe(
        time_df_display_jtt.sort_values(by="Date", ascending=False),
    ...
    if not users_df.empty:
        users_with_full = users_df.copy()
        users_with_full['FullName'] = users_with_full.apply(_full_name, axis=1)
        contractor_names_for_filter_mu = users_with_full[users_with_full['Role'].isin(['Contractor', 'Admin', 'Manager'])]['FullName'].astype(str).str.strip().unique()
    ...
    if current_user_role_val == 'Contractor':
        materials_df_display_mu = materials_df_display_mu[materials_df_display_mu['Contractor'].astype(str).str.strip() == current_user_fullname_val]
        ...
        if selected_contractor_mu_disp_filter != "All Contractors":
            materials_df_display_mu = materials_df_display_mu[materials_df_display_mu['Contractor'].astype(str).str.strip() == selected_contractor_mu_disp_filter]
    else:  # Admin, Manager
        selected_contractor_mu_disp_filter = st.selectbox("Filter by Contractor:", options=contractor_choices_mu_filter,
    ...
    if selected_job_mu_disp_filter != "All Jobs":
        materials_df_display_mu = materials_df_display_mu[materials_df_display_mu['Job'].astype(str).str.strip() == selected_job_mu_disp_filter]
    ...
    if current_user_role_val in ['Contractor', 'Admin', 'Manager']:
        ...
        with st.form("new_material_entry_form_mu_section", clear_on_submit=True):
            ...
            if selected_job_name_mu_form_val and selected_job_name_mu_form_val != "Select Job":
                ...
                st.text_input("Client (Auto-filled)", value=str(client_name_mu_form_val), disabled=True, key="mu_form_client_auto")
            else:
            ...
            if st.form_submit_button("Add Material Entry"):
                if not (material_name_mu_form and amount_mu_form is not None and contractor_input_mu_form_val and
                    ...
                    new_material_record = {
                        ...
                        'Contractor': str(contractor_input_mu_form_val).strip(),
                        'Client': str(client_name_mu_form_val).strip(),
                        'Job': str(selected_job_name_mu_form_val).strip(),
                        'Date Used': date_used_mu_form_val,
                        'Amount': float(amount_mu_form),
    ...
    display_paginated_dataframe(
        materials_df_display_mu.sort_values(by="Date Used", ascending=False),
    ...
    if current_user_role_val in ['Contractor', 'Manager', 'Admin']:
        ...
        if selected_job_ur_form != "Select Job":
            ...
            st.text_input("Client Name (Auto-filled)", value=str(client_name_ur_form), disabled=True, key="ur_form_client_auto")
        else:
        ...
        with st.form("new_receipt_form_ur_section", clear_on_submit=True):
            ...
            if not users_df.empty:
                users_with_full = users_df.copy()
                users_with_full['FullName'] = users_with_full.apply(_full_name, axis=1)
                contractor_choices_ur_list = sorted(list(users_with_full[users_with_full['Role'].isin(['Contractor', 'Admin', 'Manager'])]['FullName'].astype(str).unique()))
            else:
            ...
            if st.form_submit_button("Save Receipt Information"):
                if not (contractor_name_ur_input and selected_job_ur_form != "Select Job" and payor_query_ur_text.strip() and
                    ...
                    with st.spinner("Uploading file and saving info..."):
                        ...
                        if receipt_link:
                            new_receipt_record = {
                                'Contractor Name': str(contractor_name_ur_input).strip(),
                                'Client Name': str(client_name_ur_form).strip(),
                                'Job Name': str(selected_job_ur_form).strip(),
                                'Payor': payor_query_ur_text.strip(),
                                'Amount': float(amount_ur_input_val),
    ...
    client_list_ur_view_filter = ["All Clients"] + sorted(list(
        receipts_df_display_main_ur['Client Name'].astype(str).str.strip().replace('', np.nan).dropna().unique()
    ))
    sel_client_ur_view = st.selectbox(
        "Filter by Client (View):", client_list_ur_view_filter,
    ...
    job_list_ur_view_filter = ["All Jobs"] + sorted(list(
        receipts_df_display_main_ur['Job Name'].astype(str).str.strip().replace('', np.nan).dropna().unique()
    ))
    ...
    if not dp_df_display_dpl.empty and not jobs_df.empty:
        ...
        job_options_dpl_filter = ["All Jobs"] + sorted(list(
            dp_df_display_dpl['Job Name'].astype(str).str.strip().replace('Unknown Job', '').replace('', np.nan).dropna().unique()
        ))
    else:
    ...
    if selected_job_dpl_filter != "All Jobs":
        dp_df_display_dpl = dp_df_display_dpl[dp_df_display_dpl['Job Name'].astype(str).str.strip() == selected_job_dpl_filter]
    ...
    display_paginated_dataframe(
        dp_df_display_dpl[display_cols_dpl].sort_values(by="DateReceived", ascending=False),
        "dpl_paginated_log", 10,
        col_config={
            "DownPaymentID": None, "JobUniqueID": None,
                ...
                if selected_job_dpl_form == "Select Job" or not dp_date_dpl_form or not dp_amount_dpl_form or not dp_method_dpl_form:
                    ...
                    if not job_data_dpl_series.empty:
                        ...
                        new_dp_record = {
                            ...
                            'Amount': float(dp_amount_dpl_form),
    ...
    if selected_job_jfu_context != "Select Job to View/Upload Files":
        ...
        st.text_input("Client Name (for selected job):", value=str(client_name_jfu_context), disabled=True, key="jfu_client_context_auto")
    else:
    ...
    if selected_job_jfu_context != "Select Job to View/Upload Files" and job_uid_jfu_context:
        ...
        if not job_files_to_display_jfu.empty:
            ...
            display_paginated_dataframe(
                job_files_to_display_jfu.sort_values(by="UploadDate", ascending=False),
    ...
    if current_user_role_val in ['Admin', 'Manager']:
        ...
        clients_available_ig = ["Select Client"] + (sorted(
            list(jobs_df['Client'].astype(str).str.strip().replace('', np.nan).dropna().unique())
        ) if not jobs_df.empty else [])
        selected_client_for_doc_ig = st.selectbox("Client:", clients_available_ig, key="ig_client_doc_select")
        ...
        # Auto-populate address and description
        ...
        last_num = 499
        if doc_type_selected_ig == "Estimate" and not estimates_df.empty and 'DocNumber' in estimates_df.columns:
            numeric_est_nums = pd.to_numeric(estimates_df['DocNumber'].astype(str).str.replace('EST-', '', regex=False), errors='coerce').dropna()
            if not numeric_est_nums.empty: last_num = int(numeric_est_nums.max())
        elif doc_type_selected_ig == "Invoice" and not invoices_df.empty and 'DocNumber' in invoices_df.columns:
            numeric_inv_nums = pd.to_numeric(invoices_df['DocNumber'].astype(str).str.replace('INV-', '', regex=False), errors='coerce').dropna()
            if not numeric_inv_nums.empty: last_num = int(numeric_inv_nums.max())
        ...
        if selected_job_data_for_doc_ig is not None:
            ...
            if cfg_inc_job_est_mat_cost:
                val = float(job_details_for_items_ig.get('Estimated Materials Cost', 0.0))
            ...
            if cfg_inc_job_est_time:
                hrs = float(job_details_for_items_ig.get('Estimated Hours', 0.0)); rate = float(st.session_state.inv_est_time_job_rate)
            ...
            if cfg_inc_job_actual_mat_cost_total:
                m_cost = float(materials_df[materials_df['JobUniqueID'] == job_uid_for_items_ig]['Amount'].sum()) if not materials_df.empty else 0.0
                r_cost = float(receipts_df[receipts_df['JobUniqueID'] == job_uid_for_items_ig]['Amount'].sum()) if not receipts_df.empty else 0.0
            ...
            if cfg_inc_records_detailed_time:
                if not job_time_df.empty:
                    time_summary_ig = job_time_df[job_time_df['JobUniqueID'] == job_uid_for_items_ig].groupby('Contractor', dropna=True)['Time Duration (Hours)'].sum().reset_index()
                    for _, row_t_ig in time_summary_ig.iterrows():
                        q = float(row_t_ig['Time Duration (Hours)'])
                        auto_items_for_current_invoice_ig.append({'description': f"Labor: {row_t_ig['Contractor']}", 'quantity': q, 'unit_price': 50.0, 'total': q * 50.0, 'source': 'auto'})
            if cfg_inc_records_detailed_materials:
                if not materials_df.empty:
                    mat_summary_ig = materials_df[materials_df['JobUniqueID'] == job_uid_for_items_ig].groupby('Material', dropna=True)['Amount'].sum().reset_index()
                    for _, row_m_ig in mat_summary_ig.iterrows():
                        amt = float(row_m_ig['Amount'])
                        auto_items_for_current_invoice_ig.append({'description': f"Material: {row_m_ig['Material']}", 'quantity': 1.0, 'unit_price': amt, 'total': amt, 'source': 'auto'})
            if cfg_inc_records_down_payments:
                if not down_payments_df.empty:
        ...
        # Combine auto-generated with manual items
        ...
        for idx_li, item_li_ig in enumerate(st.session_state.invoice_line_items):
            ...
            if item_li_ig.get('source') == 'manual':
                ...
                qty_input_val = row_cols_display_ig[1].number_input("qty", value=float(item_li_ig['quantity']), key=f"qty_{row_item_key_ig}", label_visibility="collapsed")
                price_input_val = row_cols_display_ig[2].number_input("price", value=float(item_li_ig['unit_price']), format="%.2f", key=f"price_{row_item_key_ig}", label_visibility="collapsed")
                current_total_val_ig = float(qty_input_val) * float(price_input_val)
                ...
                st.session_state.invoice_line_items[idx_li] = {'description': desc_input_val, 'quantity': float(qty_input_val), 'unit_price': float(price_input_val), 'total': current_total_val_ig, 'source': 'manual'}
            else:
                # Read-only auto items with consistent formatting
                ...
                with row_cols_display_ig[1]: st.markdown(f"<div style='text-align: right; height: 38px; padding-top: 8px;'>{format_hours(item_li_ig.get('quantity', 0.0), 2)}</div>", unsafe_allow_html=True)
                with row_cols_display_ig[2]: st.markdown(f"<div style='text-align: right; height: 38px; padding-top: 8px;'>{format_currency(item_li_ig.get('unit_price', 0.0))}</div>", unsafe_allow_html=True)
                with row_cols_display_ig[3]: st.markdown(f"<div style='text-align: right; height: 38px; padding-top: 8px;'>{format_currency(item_li_ig.get('total', 0.0))}</div>", unsafe_allow_html=True)
        ...
        # Totals
        final_subtotal_ig = sum(float(item.get('total', 0.0)) for item in st.session_state.invoice_line_items)
        final_tax_amount_ig = final_subtotal_ig * (float(tax_rate_input_ig) / 100.0)
        final_grand_total_ig = final_subtotal_ig + final_tax_amount_ig
        st.markdown("---")
        st.markdown(f"#### Subtotal: {format_currency(final_subtotal_ig)}")
        st.markdown(f"#### Tax ({tax_rate_input_ig}%): {format_currency(final_tax_amount_ig)}")
        ...
        if st.button(f"Generate {doc_type_selected_ig} PDF", key="ig_final_generate_pdf_btn_ULTIMATE_DEBUG", type="primary"):
            with st.spinner("RUNNING FINAL PDF TEST..."):
                try:
                    ...
                    # fpdf2: get bytes as latin-1 encoded string buffer
                    pdf_output_bytes = pdf_gen_doc.output(dest="S").encode("latin-1")

                    if isinstance(pdf_output_bytes, (bytes, bytearray)) and len(pdf_output_bytes) > 0:
                        st.success("ULTIMATE DEBUG TEST PASSED!")
                        st.download_button("Download Test PDF", pdf_output_bytes, "test.pdf", "application/pdf")
                    else:
                        st.error("ULTIMATE DEBUG TEST FAILED: The fpdf2 library did not produce a byte buffer.")
                except Exception as e:
    ...
    if current_user_role_val in ['Admin', 'Manager', 'Client Viewer']:
        ...
        if current_user_role_val == 'Client Viewer' and associated_client_name_val:
            report_jobs_df_ra_filter = report_jobs_df_ra_filter[report_jobs_df_ra_filter['Client'].astype(str).str.strip() == associated_client_name_val.strip()]

        # Client Filter
        if current_user_role_val != 'Client Viewer':
            report_client_options_ra = ["All Clients"]
            if not jobs_df.empty:
                report_client_options_ra.extend(sorted(list(jobs_df['Client'].astype(str).str.strip().replace('', np.nan).dropna().unique())))
        else:
            report_client_options_ra = [associated_client_name_val] if associated_client_name_val else ["No Associated Client"]

        selected_report_client_ra = st.selectbox(
            "Filter Reports by Client:", report_client_options_ra,
            key="ra_client_filter",
            index=0 if current_user_role_val == 'Client Viewer'
            else (report_client_options_ra.index(st.session_state.get("ra_client_filter_val", report_client_options_ra[0]))
                  if st.session_state.get("ra_client_filter_val") in report_client_options_ra else 0)
        )
        if current_user_role_val != 'Client Viewer':
            st.session_state.ra_client_filter_val = selected_report_client_ra

        # Job Filter
        jobs_for_report_job_filter_ra = report_jobs_df_ra_filter.copy()
        if selected_report_client_ra != "All Clients":
            jobs_for_report_job_filter_ra = jobs_for_report_job_filter_ra[
                jobs_for_report_job_filter_ra['Client'].astype(str).str.strip() == selected_report_client_ra.strip()
            ]

        report_job_options_ra = ["All Jobs"] + sorted(list(
            jobs_for_report_job_filter_ra['Job Name'].astype(str).str.strip().replace('', np.nan).dropna().unique()
        ))
        selected_report_job_ra = st.selectbox(
            "Filter Reports by Job:", report_job_options_ra,
            key="ra_job_filter",
            index=report_job_options_ra.index(st.session_state.get("ra_job_filter_val", report_job_options_ra[0]))
            if st.session_state.get("ra_job_filter_val") in report_job_options_ra else 0
        )
        ...
        if selected_report_client_ra != "All Clients":
            filtered_jobs_ra = filtered_jobs_ra[filtered_jobs_ra['Client'].astype(str).str.strip() == selected_report_client_ra.strip()]
            filtered_time_ra = filtered_time_ra[filtered_time_ra['Client'].astype(str).str.strip() == selected_report_client_ra.strip()]
            filtered_materials_ra = filtered_materials_ra[filtered_materials_ra['Client'].astype(str).str.strip() == selected_report_client_ra.strip()]
            filtered_receipts_ra = filtered_receipts_ra[filtered_receipts_ra['Client Name'].astype(str).str.strip() == selected_report_client_ra.strip()]

        job_uid_for_report_filter = None
        if selected_report_job_ra != "All Jobs":
            if not filtered_jobs_ra[filtered_jobs_ra['Job Name'].astype(str).str.strip() == selected_report_job_ra.strip()].empty:
                job_uid_for_report_filter = filtered_jobs_ra[filtered_jobs_ra['Job Name'].astype(str).str.strip() == selected_report_job_ra.strip()]['UniqueID'].iloc[0]
            filtered_jobs_ra = filtered_jobs_ra[filtered_jobs_ra['Job Name'].astype(str).str.strip() == selected_report_job_ra.strip()]
            ...
            elif selected_report_job_ra != "All Jobs":
        ...
        with report_tab1:
            ...
            if not filtered_time_ra.empty:
                ...
                if not daily_hours_ra.empty:
                    daily_sum_ra = daily_hours_ra.groupby(['Contractor', 'Date'], dropna=True)['Time Duration (Hours)'].sum().reset_index()
                    if not daily_sum_ra.empty:
                        avg_daily_duration_ra = daily_sum_ra.groupby('Contractor', dropna=True)['Time Duration (Hours)'].mean().reset_index()
                        avg_daily_duration_ra = avg_daily_duration_ra.sort_values(by='Time Duration (Hours)', ascending=False)

                        fig_avg_duration_ra = px.bar(
                            avg_daily_duration_ra, x='Contractor', y='Time Duration (Hours)',
                        ...
                        st.info("No aggregated daily work hour data for contractors based on filters.")
                else:
                    st.info("No valid time entries with dates to calculate average daily duration.")
            else:
                st.info("Time tracking data insufficient for 'Average Daily Duration' chart.")

            st.markdown("#### Top Materials by Cost (from Material Usage Logs)")
            if not filtered_materials_ra.empty:
                material_costs_ra = filtered_materials_ra.groupby('Material', dropna=True)['Amount'].sum().reset_index()
                ...
                if not top_materials_ra.empty:
                    fig_top_materials_ra = px.bar(
                        top_materials_ra, x='Material', y='Amount', text='Amount',
                    ...
                    st.info("No material cost data available based on selected filters.")
            else:
                st.info("Material usage data insufficient for 'Top Materials' chart.")
        ...
        with report_tab2:
            ...
            if not filtered_jobs_ra.empty:
                ...
                if not filtered_time_ra.empty:
                    actual_hours_agg_ra = filtered_time_ra.groupby('JobUniqueID', dropna=True)['Time Duration (Hours)'].sum().reset_index()
                ...
                if not filtered_materials_ra.empty:
                    actual_mat_usage_cost_ra = filtered_materials_ra.groupby('JobUniqueID', dropna=True)['Amount'].sum()
                actual_receipts_cost_ra = pd.Series(dtype='float')
                if not filtered_receipts_ra.empty:
                    actual_receipts_cost_ra = filtered_receipts_ra.groupby('JobUniqueID', dropna=True)['Amount'].sum()
                ...
                st.dataframe(
                    job_perf_df_ra[display_cols_perf_ra].style.format({
                ...
                if not job_perf_df_ra.empty:
                    ...
                    fig_hours_perf_ra = px.bar(
                        melted_hours_perf_ra, x='Job Name', y='Hours', color='Metric', barmode='group',
                    ...
                    fig_mats_perf_ra = px.bar(
                        melted_mats_perf_ra, x='Job Name', y='Cost', color='Metric', barmode='group',
                    ...
                    st.info("No job performance data to display charts based on current filters.")
            else:
                st.info("No job data available for performance analysis based on selected filters.")
    else:
